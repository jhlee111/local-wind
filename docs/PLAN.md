...
- **M2** 진행중 (2026-07-12 인프라 완료): 공개 리포 [github.com/jhlee111/local-wind](https://github.com/jhlee111/local-wind) + **라이브 사이트 <https://local-wind.pages.dev>** (wrangler 첫 배포 ✅). Actions cron 활성화 — bake-wind(매시 :20, bake→build→CF 배포) + collect-obs(매시 :07, 관측→main 커밋). ⚠️ **CI의 CF 배포는 시크릿 대기**: 사용자가 `CLOUDFLARE_API_TOKEN`(Pages Edit 권한)·`CLOUDFLARE_ACCOUNT_ID`를 `gh secret set`으로 직접 설정해야 매시 자동 배포 시작(그 전까지 배포 스텝 자동 skip, 사이트는 마지막 수동 배포본 유지). **잔여 DoD**: 시크릿 설정 → 자동 갱신 확인, 속도 컬러 오버레이 + Cabrillo 스팟 마커 (dataviz 팔레트)
...
- basemap(OpenFreeMap) 로드 실패/차단 시에도 바람 레이어가 뜨도록 fallback 인라인 스타일 + 6s 타임아웃 가드 (`web/src/main.ts`).
