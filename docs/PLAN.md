...
- **M2.6 — UX 리디자인** (2026-07-12 진행 중): 단일 타임라인(D12) + 반응형(D13-D15). 설계 문서: [ux-redesign.md](ux-redesign.md). 단계: **UX-1 ✅** → **UX-2 ✅ (2026-07-12)** → UX-3(테이블↔차트 연동) → UX-4(모바일 하단 시트) → UX-5(임의지점 Open-Meteo 테이블+폴리시). 각 단계 독립 배포 가능
