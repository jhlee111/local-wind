...
- **완료**: M1(파이프라인) · M1.5(관측 수집) · M2(라이브+자동화+컬러 오버레이+스팟 패널) · M2.5a(클릭-anywhere) · M2.5b(주간 테이블) + 차트 예보 거스트 점
- **다음 = M2.6 UX 리디자인** (계획 완료·구현 미착수): 설계 [ux-redesign.md](ux-redesign.md) — 단일 selectedTime(D12), 모바일 하단 시트(D13), 팔레트 스크러버(D14), 범례 코너(D15). 단계 UX-1~5, **파일 단위 구현 노트까지 문서에 있음**
